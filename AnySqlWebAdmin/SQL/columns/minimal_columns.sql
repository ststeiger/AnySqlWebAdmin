...
-- insert (columnlist) 
-- foreach noncomputed column values ( valuelist)
-- select t1.columnlist from @name AS t1
-- left join table_name ON table_name .pk = t1.pk
-- where t1.pk is null 
-- select name, definition,* from sys.computed_columns

-- update table_name set fieldList - PK 
-- from table_name inner join @name as t1 on t1.pk = table_name.pk
